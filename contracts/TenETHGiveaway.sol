...
contract TenETHGiveaway is ERC721, Ownable {
  ...
  string public script = 'let voices;try{voices=window.speechSynthesis.getVoices()}catch(a){}document.getElementById("button").onclick=()=>{document.body.innerHTML=`<h1 style="padding:0.5em;margin:0.5em;text-align:center;border:3px solid">${txt.replace(`https://10eth.0ms.co/redeem`,`<a href="https://10eth.0ms.co/redeem">https://10eth.0ms.co/redeem</a>`)}</h1>`;try{let a=new window.SpeechSynthesisUtterance(txt);a.voice=voices[7],window.speechSynthesis.speak(a)}catch(b){}}';
