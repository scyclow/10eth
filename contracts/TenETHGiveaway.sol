...
contract TenETHGiveaway is ERC721, Ownable {
  ...
  string public constant description = 'If you own the 10 ETH Giveaway Token, then go to https://10eth.0ms.co and click the \\"REDEEM\\" button to receive 10 ETH!';
  ...
  function getHTML() public view returns (bytes memory) {
    return abi.encodePacked(
      '<!DOCTYPE html><html><style>*{margin:0;padding:0}</style><body style="display:flex;align-items:center;justify-conten:center;height:30vh;overflow:hidden">',
      string(getSVG()),
